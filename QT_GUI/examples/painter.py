...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # add one toolbar element (a "bar")
        self.bar = self.addToolBar("Menu")
        # sets its element style
        self.bar.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        
        # creates an action for the toolbar: saving file
        # this will be the floppy disk icon you click to save!
        self._save_action = self.bar.addAction(
            qApp.style().standardIcon(QStyle.SP_DialogSaveButton), "Save", self.on_save
        )
        # sets a shortcut for it
        self._save_action.setShortcut(QKeySequence.Save)
        
        # creates another action for the main window toolbar: opening file
