...
class Stimulus():
    def __init__(self, stimulus_number):
        ...
        self.description = {"picture path" : "", "audio path" : "", "audio cue path" : "", "stimulus duration" : 0} 
...
class ExperimentSetupWindow(QWidget):
    ...
    def unpack(self, stimulus):
        ...
        for key in stimulus.description.keys():
            text = text + f"\t{key}\t\t{stimulus.description[key]}\n"

        return text

    def upload_image_button_clicked(self):
        stimulus = self.stimuli_list[ self.list_widget.currentRow() ]
        print("opening browse stimulus dialog for stimulus {}".format(stimulus.stimulus_number))
        
        file_name = QFileDialog.getOpenFileName(parent = self, caption = "Browse Audio", dir = stimulus.data_path, filter = "")[0]   
            
        if len(file_name)>0:
            stimulus.description["picture path"] = file_name
            print("a file was selected: ", stimulus.description["picture path"])
            self.update_list_widget()
    ...
    def upload_audio_button_clicked(self):
        ...
        stimulus = self.stimuli_list[ self.list_widget.currentRow() ]
        print("opening audio browse for stimulus {}".format(stimulus.stimulus_number))
        
        file_name = QFileDialog.getOpenFileName(parent = self, caption = "Browse Audio", dir = stimulus.data_path, filter = "")[0]   
            
        if len(file_name)>0:
            stimulus.description["audio path"] = file_name
            print("a file was selected: ", stimulus.description["audio path"])
            self.update_list_widget()
    ...
    def upload_audio_cue_button_clicked(self):
        stimulus = self.stimuli_list[ self.list_widget.currentRow() ]
        print("opening audio cue browse for stimulus {}".format(stimulus.stimulus_number))
        
        file_name = QFileDialog.getOpenFileName(parent = self, caption = "Browse Audio Cue", dir = stimulus.data_path, filter = "")[0]   
            
        if len(file_name)>0:
            stimulus.description["audio cue path"] = file_name
            print("a file was selected: ", stimulus.description["audio cue path"])
            self.update_list_widget()
