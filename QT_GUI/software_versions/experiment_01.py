...
class Experiment(QWidget):
    def __init__(self):
        ...
        IMPORTANT CONSIDERATIONS
            I NEED TO BE CAREFUL WHEN IMPLEMENTING THIS, BECAUSE I'LL NEED A VERSION FOR ALL EXPERIMENTS, SO MAYBE HAVE A GENERAL PATIENT SETUP AND THEN SOMETHING RELATED TO THE SPECIFIC EXPERIMENT?
        """

class PatientData(QWidget):
    """
    compile a dictionary of values. follows "title, description"
        name: ...
        surname: ...
        date of birth: ...
    NOTE: there must be a function that allows to add some more like
        add one -> title, description
    """
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Patient Data Setup")

        self.info = {}

class MotionImagerySetup(QWidget):
        """
        setup parameters for motion imagery
            define a sequence of elements
            each element can be an image or a sound!
            each element has a duration
        user should be able to have something like
            add element: opens an image or sound and loads it
            set duration: on the right of the add element

        """
        def __init__(self):
            super().__init__()

            self.setWindowTitle("MI setup")

            self.info = {}

            layout_ext = QHBoxLayout() # external
            layout_int = QVBoxLayout() # internal
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.MI_setting_window = MotionImagerySetup()
    ...
    def open_MI_setting_window(self):
        if self.MI_setting_window.isVisible():
            pass
        else:
            self.MI_setting_window.show()
