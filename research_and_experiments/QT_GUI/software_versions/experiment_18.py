...
from PySide6.QtCore import QStandardPaths, Qt, QSize, QRunnable, Slot, QThreadPool, QTimer, QUrl
...
class EditStimulus(QDialog):
    ...
    def clear_selected_item(self): # TODO
        #list(self.description.keys())[self.info_text.currentRow()] = ""
        
        key = self.info_text.currentItem().text().split(" ")[0].split("\t")[0]
        print("setting new key: {}".format (key) )

        self.new_description[ key ] = "" # updates the information corresponding to the seleted row
        self.update_info_text()
...
class ExperimentSetupWindow(QWidget):
    ...
    def __init__(self):
        ...
        clear_b.clicked.connect(self.clear_stimulus_data)
...
class ExperimentRunDialog(QDialog):
    ...
    def __init__(self, playlist_info, stream_info, project_folder, parent=None):
        ...
        self.sequence = [ int(x) for x in self.sequence ]
        ...
        self.unpacked_stimuli = self.unpack_stimuli(playlist_info["Stimuli"])
        
        # timestamps!
        self.timestamps = [] # save both stimulus type and timestamp
    ...
    def unpack_stimuli(self,stimuli_list): # stimuli_list is a list of Stimulus objects
        ...
        unpacked_stimuli = [] # each stimulus is  list of 3 (or less, depending on how many rest images, before or after). each of the 3 elements is a dictionary with keys image, sound
        for stimulus in stimuli_list: # stimulus is a Stimulus object. access it's paths using stimulus.description[key]
            ...
            #  NOTE: in order, append stimulus, rest before, rest after
            stimulus_unpacked.append( { "picture" : stimulus.description["rest_picture(before)"], "audio" : stimulus.description["audio_cue(before)"], "duration" : self.playlist_info["Rest Duration"]  } )
            ...
            stimulus_unpacked.append( { "picture" : stimulus.description["rest_picture(after)"], "audio" : stimulus.description["audio_cue(after)"], "duration" : self.playlist_info["Rest Duration"] } )
            print(f"Stimulus unpacked: {stimulus_unpacked}")

            unpacked_stimuli.append( stimulus_unpacked ) 
                # unpacked stimuli is a list -> select ith element with unpacked_stimuli[i]
                    # each element is a list of dictionaries

        """unpack sequence as well """
        # recreate the stimuli sequence with tuples (i,j), with i: stimulus number, j: part of the stimulus you want to play. NOTE: this triplicates the sequence! but whatever! QUESTION: too bad design?
        # as defined in __init__() -> self.unpacked_sequence = []
       

        for i in self.sequence:
            if unpacked_stimuli[i][0]["picture"] != "" or unpacked_stimuli[i][0]["audio"] != "": # if rest before audio or video is there
                self.unpacked_sequence.append( tuple((int(i),0)) )  
            if unpacked_stimuli[i][1]["picture"] != "" or unpacked_stimuli[i][1]["audio"] != "": # if stimulus audio or video is there
                self.unpacked_sequence.append( tuple((int(i),1)) )
            if unpacked_stimuli[i][2]["picture"] != "" or unpacked_stimuli[i][2]["audio"] != "": # if rest after audio or video is there
                self.unpacked_sequence.append( tuple((int(i),2)) )
        
        print("sequence unpacked: {}".format(self.unpacked_sequence))

        return unpacked_stimuli # list of 2, each a list of 3 dictionaries!
    ...
    def run_experiment(self):
        ...
        self.thread_pool.start(self.thread1) # QUESTION: CHECK IF YOU NEED ANY PAUSES to sych with recording, or add some more time
        self.play_next()

    def close_and_save_experiment(self):
    ...
    def play_next(self):
        ...
        each index corresponds to a stimulus structure in self.unpacked_stimuli,
        ...
        """WHEN TO STOP? when the self."""
        if self.playlist_index == len(self.unpacked_sequence):
            self.timer.stop()

            self.close_and_save_experiment()

        else:
            index = self.playlist_index # check playlist index: self.playlist_index -> step it up when you're done! in this function!

            # self.unpacked_sequence contains a tuple (i,j) that tells you 
            #   i: stimulus to play, find it in self.unpacked_stimuli[i] -> it's a list of dictionaries!
            #   j: rest, stimulus, rest after: as a dictionary, find it in self.unpacked_stimuli[i][j]

            i,j = self.unpacked_sequence[ index ]

            """
                i: stimulus corresponding to self.unpacked_stimuli[i]
                j: tells you which stimulus (rest before, stimulus,rest after) you wanna play. select it as self.unpacked_stimuli[i][j]
                select image, audio and duration as
                    self.unpacked_stimuli[i][j]["picture"]
                    self.unpacked_stimuli[i][j]["audio"]
                    self.unpacked_stimuli[i][j]["duration"]
            """
            image = self.unpacked_stimuli[i][j]["picture"]
            audio = self.unpacked_stimuli[i][j]["audio"]
            duration = int(self.unpacked_stimuli[i][j]["duration"])

            #print(f"\nPlaying \n\timage {image}, \n\taudio {audio}\n\tduration: {duration}")

            self.timer.setInterval(1000*duration)
            self.image.setPixmap(QPixmap(image))
            self.audio.setSource(QUrl.fromLocalFile(audio))
            self.audio.play()
            self.playlist_index += 1

            # setup timestamp
            # IF I am displaying a stimulus: check j == 1
            # append the class index: i !
            if j==1:
                self.timestamps.append( tuple( (i, local_clock()) ) )
    ...
    def thread1(self):
        ...
        text = "Theoretically needed samples = sampling_rate*duration = {}\n".format( self.stream_info.nominal_srate() * duration )
        text = text + "Acquired samples: {}\n\n".format( len(samples) )
        text = text + f"Acquisition started at {start}" + f"\nFirst sample acquired at {samples[0][0]}" + f"\nLast sample acquired at {samples[-1][0]}" + f"\nAcquisition ended at {start+elapsed_time}\n\n"
        ...
        # self.thread_pool.stop(self.thread1) QUESTION: I don't need this right?
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        save_experiment_actiion = QAction(QIcon.fromTheme("application-exit"),"Quit", self)
    ...
    def approve_and_run_experiment(self):
        ...
        if experiment.exec_():
            print("Experiment Successful. Returning to main GUI")
        else: 
