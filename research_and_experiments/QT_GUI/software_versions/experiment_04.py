...
# create a custom widget you can use in the experiment setup, when choosing the stimuli
class StimulusWidget_OLD(QWidget):
    ...
    def __init__(self, widget_number):
        ...
        layout.addWidget(QLabel("Stimulus {}".format(widget_number)))
    ...
    def browse_button_clicked(self):
        ...
        file_name = QFileDialog.getOpenFileName(parent = self, caption = "Open File", dir = self.data_path, filter = "")[0]   
            #QUESTION: I cannot make the filter work! I need also audio!

        if len(file_name[0])>0:
            self.stimulus = file_name
            print("a file was selected: ", self.stimulus)
            self.name_label.setText( file_name.split("/")[-1] ) # updates the label
            
        else:
            print("No file was selected")


    #  QUESTION: HOW DO I MAKE IT INTERACTIVE, IN THE SENSE THAT I CAN ADD/REMOVE STIMULI?
...
class StimulusWidget(QWidget):
    ...
    def __init__(self, widget_number):
        ...
        # store name of selected stimulus
        self.description = {"picture path" : "", "audio path" : ""} # QUESTION: better to use a dictionary here
        self.picture_path = ""
        self.audio_path = ""
...
class ExperimentSetupWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.MI_parameters = {} # QUESTION: should I even use those? i can directly update the main window metadata!
        # NOTE : stimuli list used when opening stimuli
        self.MI_stimuli = []
        self.MI_stimuli_index = 0 
        self.MI_sequence = [] # list of indices relative to the MI classes. randomised sequence of those QUESTION: use this in the filal approve and run page!
        self.SSVEP_parameters = {} #NOTE: note used yet

        # button to accept experiment settings
        ...
        self.settings_form_elements = { "Stimulus duration" : self.stimulus_duration, "Rest time" : self.rest_time, "Number of runs per stimulus" : self.runs_per_class }

        for key in self.settings_form_elements.keys():
            self.MI_form_layout.addRow( key, self.settings_form_elements[key] )


        # MOTOR IMAGERY STIMULI 
        
        self.stimuli_layout = QVBoxLayout()
        
        # add custom widgets for browsing stimuli data, as a list # NOTE: ADD HERE MANUALLY STIMULI, AS I DON'T YET KNOW HOW TO REMOVE THEM!
        self.stimuli_list = [ StimulusWidget(widget_number = 0), StimulusWidget(widget_number = 1) ]
...
class ApproveExperimentWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.generate_sequence_button = QPushButton("Generate a random MI sequence")
        self.approve_and_run_button = QPushButton("Approve and Run")


        self.layout.addWidget(self.metadata_label)
        self.layout.addWidget(self.generate_sequence_button)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        """
        ATTRIBUTES: METADATA
        QUESTION: should I une one big dictionary? then I cannot control the order of the metadata! better assemble it at the end
        """
        self.stream_info = StreamInfo() # list of available StreamInfo objects holding metadata for LSL stream outlets
        # TODO: handle different streams using a list and some indexes. for example -> self.selected_stream = 0 # reference to the selected stream in self.stream_info list
        ...
        self.stimuli = [] # not used

        # final XDF file
        self.metadata = {} # not used. everything appended in StreamInfo object!
        ...
        self.page4.generate_sequence_button.clicked.connect(self.generate_sequence)
    ...
    def generate_sequence(self):
        ...
        # sequence of stimuli from page2.stimuli_list
        n = len(self.page2.stimuli_list) # this is a list of stimulus widget, remember!
        print("number of classes: ", n)   

        # generate a sequence of indexes
        # array of 0 or 1, their number is the number of runs per stimulus
    ...
    def update_metadata(self):
        ...
        if bool(self.patient_data):
            # QUESTION: automate this. run through dictionaries!
            text = text + "\nPatient data\n\tname\t\t{}\n\tsurname\t\t{}\n\tDate of Birth\t{}".format( self.patient_data["Name"], self.patient_data["Surname"], self.patient_data["Date of Birth"] )
